...
var doc /*: Ref(~lDocument) */ = null;

var getBrowser = function() /*: () / (&browser: locinvar_000273:Ref(~lBrowser) [Ref(~lBrowser)], &Application: locinvar_000275:Ref(~lApplication) [Ref(~lApplication)], &Components: locinvar_000277:Ref(~lComponents) [Ref(~lComponents)], &window: locinvar_000279:Ref(~lWindow) [Ref(~lWindow)], ~lPreferences: frzn, ~lExtensions: frzn, ~lApplication_extensions: frzn, ~lApplication: frzn, ~lFIApp: frzn, ~lNSIEMan: frzn, ~lNSIPBranch: frzn, ~lMFApplication: frzn, ~lMEManager: frzn, ~lMOFilePicker: frzn, ~lnsIPrefBranch: frzn, ~lMOPService: frzn, ~lnsIUpdateItem: frzn, ~lnsIExtensionManager: frzn, ~lComponents_classes: frzn, ~lComponents_interfaces: frzn, ~lComponents: frzn, ~lConsole: frzn, ~lRsDownloadHelper: frzn, ~lRsDownloadHelperP: frzn, ~lWindow: frzn, ~lnsIFilePicker: frzn, ~lFilePicker: frzn, ~lFile: frzn, ~lBrowser: frzn, ~lTab: frzn, ~lInput: frzn, ~lNodes: frzn, ~lChecked: frzn, ~lClassNames: frzn, ~lADsafeMarks: frzn, ~lNames: frzn, ~lPackedValues: frzn, ~lValues: frzn, ~lOffsetHeights: frzn, ~lOffsetWidths: frzn, ~lKeys: frzn, ~lStyles: frzn,  ~lHTMLFormElement: frzn , ~lHTMLCollection: frzn,
~lEvent: frzn, ~lEventTarget: frzn, ~lSelector: frzn, ~lRange: frzn, ~lQuery: frzn, ~lBunches: frzn, ~lBunch: frzn, ~lStyle: frzn, ~lSelection: frzn, ~lNode: frzn, ~lDocument: frzn, ~lDom: frzn, ~lF: frzn, ~lId: frzn, ~lLib: frzn, ~lURI: frzn) -> Ref(~lBrowser) / sameType */ { return browser; };
...
//};
//
//rsDownloadHelper.getFileName = function(doc) /*: (this: Top, Ref(~lDocument)) -> Str */ {
////TODO: try-catch
////	try {
...
//TODO: uncommenting breaks TCing      
//        if (num[0] == "1") {
          //PV: added "window."
