  ...
  assert(/*: Ref(~lDocument) */ (doc));
	...
	if (loc.href.indexOf("rapidshare.com")!=-1) {
      ...
      downloadcounter.style.display='none';
			...
			var ina = window.setInterval(function()   //PV: added "window."
          /*: (this: Top) -> Top */
          {
//				var zeit = doc.getElementById("zeit");
//				var num = zeit.innerHTML.match(/[\d\.]+/g);
				downloadcounter.style.display='none';
				message.innerHTML = "<B>RapidShare DownloadHelper Firefox addon message:</b><br>Now you can <u>continue browsing</u>, the file will be downloaded automatically!<br>Your automatic download will start in <font style='font-size:12px;'><b>" +
        /*num[0] + */   //PV: implicit cast 
        "</b></font> seconds.";
